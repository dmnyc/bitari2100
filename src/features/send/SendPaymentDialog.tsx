...
const SendPaymentDialog: React.FC<SendPaymentDialogProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<
    "input" | "amount" | "workflow" | "processing" | "result"
  >("input");
  ...
  const [prepareResponse, setPrepareResponse] = useState<
    import("@breeztech/breez-sdk-spark").PrepareSendPaymentResponse | null
  >(null);
  ...
  const processPaymentInputAsync = async (input: string | null = null) => {
    ...
    try {
      ...
      setPaymentInput({
        rawInput: currentInput.trim(),
        parsedInput: parseResult,
      });
      if (
        parseResult.type === "bolt11Invoice" &&
        parseResult.amountMsat &&
        parseResult.amountMsat > 0
      ) {
  ...
  const prepareSendPayment = async (
    ...
    try {
      ...
      setError(
        `Failed to prepare payment ${err instanceof Error ? err.message : "Unknown error"}`,
      );
      setCurrentStep("amount");
    } finally {
      setIsLoading(false);
  ...
  return (
    <BottomSheetContainer isOpen={isOpen} onClose={onClose}>
      <BottomSheetCard>
        <DialogHeader
          title={getDialogTitle().toUpperCase()}
          onClose={onClose}
        />
        ...
        <StepPanelGroup>
          ...
          <StepPanel isActive={currentStep === "input"}>
            <InputStep
              ...
              isLoading={isLoading}
              error={error}
              onContinue={(paymentInput) =>
                processPaymentInputAsync(paymentInput)
              }
              onScanQr={onScanQr}
          ...
          <StepPanel isActive={currentStep === "workflow"}>
            ...
            {prepareResponse &&
              prepareResponse.paymentMethod.type === "bitcoinAddress" && (
                <BitcoinWorkflow
                  method={prepareResponse.paymentMethod}
                  amountSats={prepareResponse.amount}
            ...
            {prepareResponse &&
              prepareResponse.paymentMethod.type === "sparkAddress" && (
                <SparkWorkflow
                  method={prepareResponse.paymentMethod}
                  amountSats={prepareResponse.amount}
                  onBack={() => setCurrentStep("input")}
